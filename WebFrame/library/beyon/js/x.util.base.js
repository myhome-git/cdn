...
//		}
		...
		var _default = {
			...
			error : function(xhr,type,text){
				...
				var _layer = layui.layer || layer;
				
				_layer.msg(""+text,{time:5000});
	...
	x.dataVerification = function(sourceObject,optionObject){
		if(this.typeOf(sourceObject)!="object" || this.typeOf(optionObject)!="object"){
   			return;
   		}
   		for(var i in sourceObject){
			if(x.typeOf(optionObject[i])===undefined || x.typeOf(optionObject[i])==null || optionObject[i]==""){
    ...
    //将字符串转换为16进制
    x.stringToHex = x.toJsonValueHex = function(_s){
    	var _this = this;
    	var _ret = _s;
    	var _return = "";
...
//			console.info(_params);
			...
			for(var i=0;i<_array.length;i++){
				var d = _array[i];
				this.append($("<input type='radio' name=\""+_object["name"]+"\" x-input-name='"+_object["name"]+"' x-out-name=\""+_object["name"]+"\" value=\""+d[_keys["_key"]]+"\" title='"+d[_keys["_value"]]+"' lay-filter='"+_tempLayFilterName+"' />"));
			}
		...
		layuiFormRender : function(){
			...
			if(layui && layui.form){
				...
				//如果form没有lay-filter
				var _layuiFilter = $this.attr("lay-filter") || null;
				if(!_layuiFilter){
//					$this.attr("lay-filter","temp-lay-filter-"+new Date().getTime());
//					console.warn("is not lay-filter");
				}
