...
(function($){
	$.fn.extend({
		xAccordionMenu : function(parameter){
			...
			_object.SetElementDataList = function(datalist){
				...
				$.each(datalist, function(index,el) {
					_lis += "<li class=\"x-accordion-item\">"
							...
							+"		</div>"
							+"		<ul class=\"x-accordion-children\" id=\"accordion-"+el.id+"\">"+(el.content?el.content:"")+"</ul>"
							+"	</li>";
				});
			 ...
			 * @param {Object} id
			 */
			_object.getElementContentById = function(id){
				return _object.find("#accordion-"+id);
			}
			 ...
			 * @param {Object} index
			 */
			_object.getElementContentByIndex = function(index){
				return _object.find(".x-accordion-children:eq("+index+")");
			}
