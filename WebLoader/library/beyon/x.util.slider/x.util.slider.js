        ...
        utilSlider : function(option){
			...
			var $doc = $(this),
           	...
           	var _html = "<div class=\"x-ui-mouse-drag-slider x-clearfix\">"
						...
						+"		<input type=\"text\" "+(option.inputAttr?option.inputAttr:"")+" class=\"layui-input\" autocomplete=\"off\" />"
						+"	</div>"
						+"</div>";
...
//				debugger;
				...
				var _value = _progress*((Math.abs(_default.min)+_default.max)/100);
				_value += _default.min;
				...
				if(_default.step==parseInt(_default.step)){
					_value = _value.toString().match(/^\-{0,1}\d+/);
				}else{
					_value = _value.toString().match(/^\-{0,1}\d+\.{0,1}\d{0,1}/);
				}
			...
			function setValue(value){
				...
				var _progress;
				if(_default.min<0){
					_progress = (value+_default.max)/(Math.abs(_default.min)+_default.max)*100;
				}else{
					_progress = value/(Math.abs(_default.min)+_default.max)*100;
				}
				
				_progress = _progress.toString().match(/^\-{0,1}\d+\.{0,1}\d{0,1}/).toString();
