...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function(e) {
	...
	j.prototype.config = {
		...
		if(i.startTime = (new Date).getTime(), a.url) {
			if(a.limitAuto){
				i.layMain.addClass("layui-table-loadding");
				var _limitTdHeight = 0;
				var _int = (function(){
					let _num = 0;
					let $table = i.layMain.find(">table");
					$table.find("tbody").empty();
					for(var _i=0;_i<100;_i++){
						if($table.outerHeight()>i.layMain.outerHeight()){
							if(_limitTdHeight && ($table.outerHeight()-i.layMain.outerHeight())<=_limitTdHeight/3){
							...
							$table.append("<tr><td style='border:0;'><div class='layui-table-cell' style='font-size:0;'>"+_i+"</div></td></tr>");
							if(!_limitTdHeight){
								_limitTdHeight = $table.find("tr:eq(0)").outerHeight();
							}
						}
						_num += 1;
					}
			...
			a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), t.ajax({
				...
				complete : function(){
					i.layMain.removeClass("layui-table-loadding");
				}
			})
